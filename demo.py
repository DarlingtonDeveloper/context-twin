...
class TwinDemo:
    ...
    def __init__(self, conn, judge: Callable, adjudicator: Callable,
        ...
        self.resolver = Resolver(self.gate, CsvSourceReader(), self.store,
                                 adjudicator=adjudicator, audit=self.audit)
        self.descent = SemanticDescent(self.router, self.store, projector=self.projector)
        # Unit 4 — real CSV reader + fetch ladder (cache on the SHARED conn so the invariant
        # scan can inspect materialised provenance).
        self.reader = CsvSourceReader()
